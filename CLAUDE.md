...
**Own the technical quality and traceability of every adjustment touching code you work near — yours, another Claude session's, or Manus.Ai's — and audit rather than trust on faith.** A task marked "concluído" in #290 or `docs/task-registry/` is a claim to verify against the real code before you build on it, not a fact to inherit — this session already caught two cases where a prior "done" didn't match reality (a tenant showing "Conectado" that never actually paired; a theme color override with a silent gap since its original implementation). When asked to audit, or when you touch an area another agent recently changed, reconstruct what actually happened from `git log`/`git blame`, the matching `docs/task-registry/TASK-XXXX.md`, the #290 comment thread, and the code/UI's current real behavior — not just what the documentation claims. Visibility into Manus.Ai's own work is limited to what it committed and reported in #290, never its internal process, so an "audit" of its work means checking that today's code matches what it reported, not reviewing its reasoning. Report any discrepancy you find explicitly instead of quietly working around it.
...
`src/App.tsx` owns most top-level state (tenants, current user, leads, transactions, knowledge base) and persists to `localStorage` as a cache; on load it re-fetches real data from the backend (`/api/conversations`, `/api/knowledge-base`, etc.) and overwrites the cached state. **Not everything is backend-connected yet**: `OperatorCRM.tsx` is currently a pure `localStorage`-backed mock with zero API calls — don't assume a UI surface is wired to real data without checking for an `apiFetch` call. **`FinancialDashboard.tsx` is NOT the real Financeiro tab** — corrected finding (TASK-0097, 26/08/2026): despite what an earlier version of this doc said, it's fully wired to `/api/financial/transactions` (`onAddTransaction`/etc. all persist via `apiFetch`), but it is never rendered anywhere — `App.tsx` only mentions it in a comment. The tab the user actually sees (`activeTab === 'financial'`/`'agenda'`) renders `AgendaFinanceiroCenter.tsx` (prop `scope: 'financial' | 'agenda'`), which has its own separate transaction table/UI. Any change to the real Financeiro/Agenda experience belongs in `AgendaFinanceiroCenter.tsx`, not `FinancialDashboard.tsx` — verify which component a tab actually renders in `App.tsx` before editing, don't assume from a file's name. `WhatsAppLeadsSim.tsx` is the real conversation view; it pushes updates via Server-Sent Events (`GET /api/conversations/stream`, `server/services/conversationEvents.ts`) rather than polling, with just a 90s safety poll as a fallback (the SSE pub/sub is in-memory, single-instance, doesn't survive a process restart). Tabs must stay mounted (CSS `display` toggle, not conditional JSX) or in-memory conversation state is lost on remount.
