...
export const TrafficCenter: React.FC = () => {
  ...
  const saveConnection = async (event: React.FormEvent) => {
    ...
    try {
      const response = await apiFetch('/api/meta-ads/connection', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adAccountId, ...(accessToken.trim() ? { accessToken: accessToken.trim() } : {}) }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || copy.saveError);
      setConnection(data.connection); setAccessToken(''); setIsEditingConnection(false); setNotice(copy.connectionSaved); await refreshMetrics();
  ...
  return <div className="space-y-6">
    ...
    {(!isConfigured || isEditingConnection) ? <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-3xl space-y-5"><div className="flex gap-3"><span className="p-2.5 h-fit rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300"><Settings2 className="w-5 h-5" /></span><div><h3 className="text-lg font-bold text-white">{isConfigured ? copy.updateReadAccess : copy.connectReadAccess}</h3><p className="mt-1 text-sm leading-relaxed text-slate-400">{copy.accessDescription}</p></div></div><form onSubmit={saveConnection} className="space-y-4"><div><label className="block mb-1.5 text-xs font-semibold text-slate-300">{copy.adAccount}</label><input value={adAccountId} onChange={(event) => setAdAccountId(event.target.value)} placeholder="act_677275869339059" required className="w-full px-3 py-2.5 rounded-xl bg-slate-950 border border-slate-700 text-slate-100 font-mono text-sm focus:outline-none focus:border-emerald-500" /></div><div><label className="block mb-1.5 text-xs font-semibold text-slate-300">{copy.readToken}</label><input type="password" value={accessToken} onChange={(event) => setAccessToken(event.target.value)} placeholder="EAAG…" required={!connection?.accessTokenSet || isEditingConnection} autoComplete="new-password" className="w-full px-3 py-2.5 rounded-xl bg-slate-950 border border-slate-700 text-slate-100 font-mono text-sm focus:outline-none focus:border-emerald-500" /><p className="mt-1.5 text-[11px] text-slate-500">{copy.permissionNote}</p></div><div className="flex flex-wrap gap-2"><button type="submit" disabled={isSaving} className="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white text-sm font-semibold flex items-center gap-2 transition-all">{isSaving ? <RefreshCw className="w-4 h-4 animate-spin" /> : <ShieldCheck className="w-4 h-4" />}{isSaving ? copy.savingConnection : copy.saveConnection}</button>{isConfigured && <button type="button" onClick={() => { setIsEditingConnection(false); setAccessToken(''); setError(null); }} className="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 text-sm font-semibold transition-all">{copy.cancel}</button>}</div></form></section> : <>
      ...
      <section className="bg-slate-900 border border-slate-800 rounded-2xl p-5"><div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3"><div><h3 className="text-base font-bold text-white flex items-center gap-2"><TrendingUp className="w-5 h-5 text-emerald-400" />{copy.creativeComparison}</h3><p className="mt-1 text-xs text-slate-400">{copy.creativeDescription}</p></div><span className="text-[11px] text-slate-500">{copy.connectedAccount} {overview.accountId}</span></div><div className="mt-4 overflow-x-auto"><table className="w-full text-left text-xs min-w-[940px]"><thead><tr className="border-b border-slate-800 text-slate-500 uppercase tracking-wide"><th className="pb-3 pr-3">{copy.creativeAd}</th><th className="pb-3 pr-3">{copy.campaign}</th><th className="pb-3 pr-3">{copy.statusLabel}</th><th className="pb-3 text-right">{copy.investment}</th><th className="pb-3 text-right">{copy.conversations}</th><th className="pb-3 text-right">{copy.costPerConversation}</th><th className="pb-3 text-right">CTR</th><th className="pb-3 pl-4">{copy.quality}</th></tr></thead><tbody className="divide-y divide-slate-800/80">{overview.ads.map((ad) => <tr key={ad.id} className="hover:bg-slate-800/40"><td className="py-3 pr-3"><p className="font-semibold text-slate-100">{ad.name}</p><p className="mt-0.5 text-[10px] text-slate-500">{ad.adSetName}</p></td><td className="py-3 pr-3 text-slate-300">{ad.campaignName}</td><td className="py-3 pr-3"><StatusBadge status={ad.deliveryStatus} /></td><td className="py-3 text-right text-slate-200">{money(ad.spend, currency, locale)}</td><td className="py-3 text-right text-slate-200">{number(ad.messagingConversations, locale)}</td><td className="py-3 text-right font-semibold text-emerald-300">{money(ad.costPerMessagingConversation, currency, locale)}</td><td className="py-3 text-right text-slate-300">{percentage(ad.ctr, locale)}</td><td className="py-3 pl-4"><p className="text-slate-300">{copy.quality}: {rank(ad.qualityRanking)}</p><p className="mt-0.5 text-[10px] text-slate-500">{copy.engagement}: {rank(ad.engagementRateRanking)}</p></td></tr>)}</tbody></table></div></section></>}
