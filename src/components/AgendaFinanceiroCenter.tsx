    ...
    <div className={`agenda-financeiro-workspace agenda-financeiro-workspace--${view} agenda-financeiro-workspace--financial-detail-${mobileDetail || 'closed'} agenda-financeiro-workspace--agenda-mobile-${mobileAgendaView} space-y-5 animate-page-enter`}>
      ...
      {view === 'agenda' && <section className="grid grid-cols-1 gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/75 p-4 shadow-lg sm:p-5">
          {/* Achado real (26/08/2026, pedido do dono do produto com print): o
              nav de mês e o banner "Dia selecionado" mostravam a mesma
              informação (selectedDateLabel) duas vezes, em dois blocos
              empilhados — ocupava espaço vertical à toa antes da grade do
              calendário aparecer, especialmente incômodo no celular. Fundido
              num único cartão: navegação de mês + dia selecionado + contagem
              de compromissos + atalho "Ir para hoje", tudo numa linha só. */}
          <div className="mb-3">
          ...
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-800 bg-slate-950 px-2 py-1.5">
            <div className="flex min-w-0 items-center gap-1">
              ...
              <div className="min-w-0 px-1">
                <p className="truncate text-xs font-bold capitalize text-slate-200">{monthLabel}</p>
                <p className="mt-0.5 truncate text-[10px] font-medium text-emerald-300">{selectedDateLabel} · {selectedDateEvents.length === 1 ? (isSpanish ? '1 cita' : '1 compromisso') : `${selectedDateEvents.length} ${isSpanish ? 'citas' : 'compromissos'}`}{selectedDate === dateInputValue(new Date()) ? ` · ${isSpanish ? 'hoy' : 'hoje'}` : ''}</p>
              </div>
