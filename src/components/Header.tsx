...
export const Header: React.FC<HeaderProps> = ({ activeTab, setActiveTab, savedCount, currentUser, onOpenLoginModal, onLogout, tenants, activeTenant, onSelectTenant }) => {
  ...
  const selectTab = (tab: ActiveTab) => { setActiveTab(tab); setIsMobileMenuOpen(false); };
  ...
  return <header className="sticky top-0 z-30 border-b border-slate-800 bg-slate-900 shadow-md">
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between gap-3 py-3 md:hidden">
        ...
        <div className="flex items-center gap-1"><button type="button" onClick={() => setLanguage(language === 'pt' ? 'es' : 'pt')} className="rounded-md border border-slate-700 px-2 py-1 text-[10px] font-bold text-slate-200" title={isSpanish ? 'Português' : 'Español'}>{isSpanish ? 'PT' : 'ES'}</button><button type="button" onClick={toggleTheme} className="rounded-md p-1.5 text-slate-300 hover:bg-slate-800" title={theme === 'dark' ? 'Modo claro' : 'Modo escuro'}>{theme === 'dark' ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}</button><button type="button" onClick={() => setIsMobileMenuOpen((value) => !value)} className="rounded-md p-1.5 text-slate-200 hover:bg-slate-800" title={copy.menu}>{isMobileMenuOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}</button></div>
      </div>
      {isMobileMenuOpen && <div className="border-t border-slate-800 pb-3 pt-2 md:hidden"><div className="flex gap-1 overflow-x-auto pb-1">{primaryNavigation.map(renderTab)}{adminNavigation.map(renderTab)}{canSeeSaasMaster && renderTab({ id: 'saas', label: copy.companies, icon: <Layers className="w-4 h-4" /> })}</div><div className="mt-3 flex items-center justify-between rounded-lg border border-slate-700 bg-slate-950 px-3 py-2"><div className="min-w-0"><p className="text-[10px] text-slate-500">{copy.activeCompany}</p><p className="truncate text-xs font-semibold text-slate-200">{activeTenant.name}</p></div>{currentUser ? <button type="button" onClick={onLogout} className="inline-flex items-center gap-1 text-xs font-semibold text-rose-300"><LogOut className="w-3.5 h-3.5" />{copy.signOut}</button> : <button type="button" onClick={onOpenLoginModal} className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-300"><User className="w-3.5 h-3.5" />{copy.signIn}</button>}</div></div>}
