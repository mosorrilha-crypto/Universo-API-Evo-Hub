  ...
  return <header className="sticky top-0 z-30 border-b border-slate-800 bg-slate-900 shadow-md" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
    <div className={headerInnerClassName}>
      <div className="flex items-center justify-between gap-3 py-3 md:hidden">
        ...
        <div className="flex items-center gap-1.5"><button type="button" onClick={() => setLanguage(language === 'pt' ? 'es' : 'pt')} className="rounded-md border border-slate-700 px-2.5 py-1 text-xs font-bold text-slate-200" title={isSpanish ? 'Português' : 'Español'}>{isSpanish ? 'PT' : 'ES'}</button><button type="button" onClick={toggleTheme} className="rounded-md p-1.5 text-slate-300 hover:bg-slate-800" title={theme === 'dark' ? 'Modo claro' : 'Modo escuro'}>{theme === 'dark' ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}</button><button type="button" onClick={() => { setIsMobileMenuOpen((value) => !value); setOpenToolsMenu(null); setIsMobileTenantMenuOpen(false); }} className="rounded-md p-1.5 text-slate-200 hover:bg-slate-800" title={copy.menu}>{isMobileMenuOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}</button></div>
      </div>
      {isMobileMenuOpen && (
        <>
          {/* TASK-0195: fecha ao tocar fora do menu — este menu nunca teve
              ...
              menu). */}
          <div className="fixed inset-0 z-20 md:hidden" onClick={() => setIsMobileMenuOpen(false)} aria-hidden="true" />
          <div
            className="mobile-nav-menu relative z-30 animate-page-enter border-t border-slate-800 pb-3 pt-2 md:hidden"
            onTouchStart={handleMobileMenuTouchStart}
            onTouchEnd={handleMobileMenuTouchEnd}
          >
            {/* TASK-0195: rótulo de seção — o mesmo padrão já usado abaixo
                pra "Empresa ativa"/"Modo visual", pra deixar claro onde a
                navegação termina e os controles de conta/aparência começam
                (antes tudo tinha o mesmo peso visual, sem nenhuma divisão). */}
        ...
        {/* TASK-0284 (pedido direto): mesmo toggle da fileira desktop
            (ver acima), só que como linha própria aqui — o mobile não tem
            os ícones soltos ao lado do seletor de idioma/tema. */}
        {currentUser && <div className="mt-2 flex items-center justify-between gap-2 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2">
          <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-500">Notificações push</span>
          <button
            type="button"
            onClick={() => void togglePush()}
            disabled={pushBusy}
            className={`inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[10px] font-bold transition-colors disabled:opacity-50 ${pushEnabled ? 'bg-emerald-500/20 text-emerald-300' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}
          >
            {pushEnabled ? <Bell className="h-3.5 w-3.5" /> : <BellOff className="h-3.5 w-3.5" />}
            {pushBusy ? 'Aguarde...' : pushEnabled ? 'Ativas' : 'Ativar'}
          </button>
        </div>}
          </div>
        </>
