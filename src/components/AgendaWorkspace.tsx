...
export function AgendaWorkspace(props: AgendaWorkspaceProps) {
  ...
  return (
    <div className="agenda-mobile-workspace space-y-3">
      <nav className="grid grid-cols-2 gap-1 rounded-2xl border border-slate-800 bg-slate-950/95 p-1.5 sm:hidden" aria-label="Visão da Agenda">
        <button type="button" onClick={() => setMobileView('today')} aria-pressed={mobileView === 'today'} className={`flex items-center justify-center gap-2 rounded-xl px-3 py-2 text-xs font-bold transition ${mobileView === 'today' ? 'bg-emerald-400 text-slate-950 shadow-sm' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}><ListChecks className="h-4 w-4" /> Hoje</button>
        <button type="button" onClick={() => setMobileView('calendar')} aria-pressed={mobileView === 'calendar'} className={`flex items-center justify-center gap-2 rounded-xl px-3 py-2 text-xs font-bold transition ${mobileView === 'calendar' ? 'bg-sky-400 text-slate-950 shadow-sm' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}><CalendarDays className="h-4 w-4" /> Calendário</button>
      </nav>
