  ...
  return (
    <div className="agenda-mobile-workspace space-y-3">
      <nav className="flex gap-1 overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950/95 p-1.5 sm:hidden" aria-label="Visão da Agenda">
        ...
        <button type="button" onClick={() => setMobileView('pending')} aria-current={mobileView === 'pending' ? 'page' : undefined} className={`flex min-w-max flex-1 items-center justify-center gap-2 rounded-xl px-3 py-2 text-xs font-bold transition ${mobileView === 'pending' ? 'bg-amber-300 text-slate-950 shadow-sm' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}><AlertCircle className="h-4 w-4" /> Pendências</button>
      </nav>
