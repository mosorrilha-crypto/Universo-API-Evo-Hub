...
- **Achado separado (não corrigido nesta tarefa):** `npm run lint` (`tsc --noEmit`) já estava quebrado no `main` antes desta sessão — dezenas de erros `string | string[]` em várias rotas, introduzidos pela migração pro Express 5 (commit `63e8218`, PR #383). Confirmado via `git stash` que não é causado por esta tarefa. Vale uma tarefa própria pra tipar `req.params`/`req.query` pós-Express 5.
