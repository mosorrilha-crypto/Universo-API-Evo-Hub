...
5. ~~**Catálogo público e troca de endereço**~~ — migration aplicada, dado corrigido pro tenant certo (`monique-teste`) e aba de edição no painel adicionada (ver "Mudanças recentes" e linha "Catálogo público por tenant" acima). Falta só conferência visual/funcional humana de `/catalogo/monique-teste` em produção antes de trocar o link usado em anúncios/bio pela Netlify.
