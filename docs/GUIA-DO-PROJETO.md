...
| Central de Qualidade e Aprendizado | 🚧 Em implementação | `QualityAuditCenter.tsx` oferece revisão de sugestões da IA, bugs, ideias de operadores, recomendações e linha do tempo. `qualityAuditStore.ts` e `qualityAudit.ts` mantêm o feedback separado por tenant. O webhook de comprovantes e as decisões humanas de pagamento geram eventos auditáveis. A migration `0040_quality_audit_learning.sql` ainda precisa ser aplicada e confirmada antes de ativar os endpoints em produção. |
...
6. **Central de Qualidade aguardando migration 0040** — o código está na branch de implementação, mas as tabelas de revisões/eventos precisam ser aplicadas e validadas antes de publicar a API.
