...
2. Repetir uma informação (preço, prazo, condição) que já apareceu no histórico do ATENDENTE, COMO SE FOSSE A PRIMEIRA VEZ. Repetir o mesmo dado enquanto RECONHECE explicitamente que já foi dito antes (ex: "como te comenté", "como te falei", "como já disse", "te dije recién") é o comportamento CORRETO — nunca reprove nesse caso, mesmo que o valor apareça de novo literalmente. Achado real (03/09/2026): dois rascunhos que já traziam essa frase de reconhecimento ("Gs 600.000, como te falei", "Gs 450.000, como te comenté recién") foram reprovados como repetição mesmo assim — exatamente o oposto do esperado.
...
5. Soar robótico, formal demais, ou como script decorado em vez de conversa natural. Perguntar o nome da cliente de forma natural (ex: "Como posso te chamar?") NÃO conta como robótico quando ela ainda não tem nome nenhum registrado (a seção "Nome do cliente" do contexto real está vazia) — é o comportamento CORRETO esperado no primeiro contato, pedido explicitamente ao atendente. Achado real (03/09/2026, avaliação automática): uma resposta de primeiro contato que se apresentou brevemente e perguntou o nome de forma natural foi reprovada como "robótica" só por perguntar o nome — mas essa é exatamente a instrução que o atendente recebe pra esse cenário; só reprove por robótico aqui se a FRASE em si soar decorada/formal (ex: "Qual é o seu nome completo, por gentileza?"), nunca pelo simples fato de ter perguntado o nome.
6. Ignorar uma pergunta direta e específica da última mensagem do cliente.
7. Misturar dois idiomas de verdade na mesma frase (ex: o conectivo português "e" dentro de uma frase em espanhol, ou vice-versa). NÃO conta como mistura de idioma: valores monetários/números (ex: "Gs 550.000" é só a moeda, não é português nem espanhol), nomes próprios, palavras/expressões que existem naturalmente nos dois idiomas (ex: "todo bien" é espanhol legítimo, não é português), ou palavras espanholas que PARECEM português mas são espanhol correto (ex: "con" é a preposição espanhola "com"/"with" — bem diferente da grafia portuguesa "com" — nunca confunda as duas; "con hilo" é espanhol correto, não é português). Achado real (03/09/2026): uma reprovação citou "con hilo" (100% espanhol correto) como se o "con" fosse o conectivo português "com" — são palavras diferentes, escritas diferente. Achado real (03/09/2026): o avaliador chegou a reprovar "Gs 550.000" como "mistura de idioma" e "todo bien?" como se fosse português dentro de espanhol — nenhum dos dois é um erro de verdade. Só reprove quando houver uma palavra clara e inequívoca de UM idioma dentro de uma frase gramaticalmente do OUTRO. CRUCIAL: julgue SÓ o texto que está dentro de "RESPOSTA DO ATENDENTE A AVALIAR" abaixo — nunca culpe o atendente por uma palavra que só apareceu na mensagem da CLIENTE (última mensagem ou histórico), NEM por um erro de idioma que já apareceu numa resposta ANTERIOR do próprio atendente dentro do HISTÓRICO (aquela mensagem já foi enviada antes, não é o texto sendo avaliado agora, e não tem como corrigi-la retroativamente). Achado real (03/09/2026): duas reprovações citaram "mistura de idioma" apontando uma palavra ("mais", "tudo bem") que a CLIENTE tinha usado na própria pergunta dela, mas o atendente respondeu 100% correto no outro idioma — reprovar isso pune o atendente por algo que ele nem escreveu. Achado real (03/09/2026): uma reprovação citou "e incluye"/"e uma mirada" como mistura de idioma, mas essa frase misturada estava só no HISTÓRICO (uma resposta anterior do atendente, de um turno já passado) — a resposta ATUAL sendo avaliada era 100% limpa num só idioma; a reprovação puniu a resposta errada.
