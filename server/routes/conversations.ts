...
export function createConversationsRouter({ authenticateToken, metaAccessToken, metaPhoneNumberId, supabaseUrl, supabaseKey }: ConversationsRouterDeps): Router {
  ...
  router.get('/api/conversations', authenticateToken, async (req: AuthenticatedRequest, res) => {
    res.json({ conversations: await listConversations(tenantOf(req)) });
  });

  router.get('/api/conversations/:phone', authenticateToken, async (req: AuthenticatedRequest, res) => {
    const conv = await getConversation(tenantOf(req), req.params.phone);
    if (!conv) return res.status(404).json({ error: 'Conversa não encontrada.' });
    res.json({ conversation: conv });
  });

  router.post('/api/conversations/:phone/send', authenticateToken, async (req: AuthenticatedRequest, res) => {
    const { text } = req.body || {};
  ...
  router.post('/api/conversations/:phone/send-media', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  router.delete('/api/conversations/:phone/history', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  router.delete('/api/conversations/:phone', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  router.delete('/api/conversations/:phone/messages/:messageId', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  router.post('/api/conversations/:phone/send-example-photo', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  router.get('/api/agent-status', authenticateToken, async (req: AuthenticatedRequest, res) => {
    res.json({ status: await getAgentStatus(tenantOf(req)) });
  });

  router.post('/api/agent-status', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  router.get('/api/knowledge-base', authenticateToken, async (req: AuthenticatedRequest, res) => {
    res.json({ knowledgeBase: await getKnowledgeBase(tenantOf(req)) });
  });

  router.post('/api/knowledge-base', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  router.get('/api/escalations', authenticateToken, async (req: AuthenticatedRequest, res) => {
    res.json({ escalations: await listEscalations(tenantOf(req)) });
  });

  router.post('/api/escalations/:id/resolve', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  router.delete('/api/escalations/:id', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  router.get('/api/quick-replies', authenticateToken, async (req: AuthenticatedRequest, res) => {
    res.json({ quickReplies: await getQuickReplies(tenantOf(req)) });
  });

  router.post('/api/quick-replies', authenticateToken, async (req: AuthenticatedRequest, res) => {
